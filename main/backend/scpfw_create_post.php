...
function SCPFWget_page_by_title_custom($page_title) {
    $args = array(
        'post_type' => 'scpfw_size_chart',
        'post_status' => 'publish',
        'name' => sanitize_title($page_title), // Convert the title to a slug format
    ...
    if ($query->have_posts()) {
        $query->the_post();
        return get_post(); // Return the post object
    } else {
            ...
            if(!isset( $_POST['scpfw_import_nonce_field'] ) || !wp_verify_nonce( $_POST['scpfw_import_nonce_field'], 'scpfw_import_nonce_action' )) {
                ...
                if(!empty($post_array)) {
                    foreach ($post_array as $key => $value) {
                        ...
                        $customPost = SCPFWget_page_by_title_custom($key);
